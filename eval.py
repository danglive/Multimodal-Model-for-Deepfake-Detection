...
def main(input_dir, output_file):
    ...
    test_data = pd.DataFrame({
        ...
        "label": [1] * len(test_videos) # Dummy label for inference
    })
    test_data.to_csv("test.csv", index=False)

    # Crop faces (metadata extraction)
    log("cropping videos")
    ...
    avg_crop_time = (time.time() - start_time) / len(test_videos)
    ...
    # If label file exists, calculate AUC for evaluation
    ...
    if os.path.exists(label_file):
        ...
        if set(label["filename"]) == set(result["filename"]):
            ...
            fpr, tpr, _ = metrics.roc_curve(merged.label, merged.probability, pos_label=1)
...
if __name__ == "__main__":
    ...
    parser.add_argument("-output", type=str, required=True, help="Output file path (e.g. /data/output/submission.csv)")
