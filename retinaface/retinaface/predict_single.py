...
class Model:
    ...
    def predict(
        ...
        with torch.no_grad():
            return self.model(torched_images)  # pylint: disable=E1102

    def predict_jsons_batch(
        self, images: np.ndarray, confidence_threshold: float = 0.7, nms_threshold: float = 0.4
    ) -> List[Dict[str, Union[List, float]]]:
        with torch.no_grad():
            original_height, original_width = images[0].shape[:2]

            transformed_images = np.array([self.transform(image=image)["image"] for image in images])

            transformed_height, transformed_width = transformed_images[0].shape[:2]
            ...
            prior_box = priorbox(
                ...
                steps=[8, 16, 32],
                clip=False,
                image_size=transformed_images[0].shape[:2],
            ).to(self.device)

            torched_images = torch.from_numpy(np.ascontiguousarray(np.transpose(transformed_images, (0, 3, 1, 2))))\
                .to(self.device)
            if self.device == "cuda":
                torched_images = torched_images.half()

            loc, conf, land = self.model(torched_images)  # pylint: disable=E1102
            ...
            for batch_idx in range(len(images)):
                ...
                boxes = decode(loc.data[batch_idx], prior_box, self.variance)

                boxes *= scale_bboxes
                scores = conf[batch_idx][:, 1]

                landmarks = decode_landm(land.data[batch_idx], prior_box, self.variance)
